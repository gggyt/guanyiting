...
class AllUserInvitationView extends React.Component {
  state = {
    disabled: false,
  }
  constructor(props) {
  ...
  getInvitation() {
     fetch(SelectInvivation,{
        ...
        body: 'invitationTitle='+this.state.invitaionTitle+'&pageNum='+this.state.nowPage+'&pageSize='+this.state.pageSize+'&getMy='+this.props.match.params.id
        ...
        <List renderHeader={() => '我发表的帖子'}>
        {
          this.state.invitationAll.map(item => {
              return(
                <Item
                arrow="horizontal"
                onClick={() => {window.location.href='/mobile/invitationDetatil/'+item.invitationId}}
              >{item.invitationTitle}</Item>
              )
          })
        }
        {this.state.invitationAll.length==0?<p style={{height:50, padding:10}}>暂无数据</p>:<p></p>}
        </List>
      </div>
...
export default AllUserInvitationView;