...
class ManageMyProblem extends React.Component {
  state = {
    disabled: false,
  }
  constructor(props) {
  ...
  getInvitation() {
     fetch(SelectProblem,{
        ...
        body: 'pageNum='+this.state.nowPage+'&pageSize='+this.state.pageSize+'&my=1'
        ...
        <List renderHeader={() => '我发表的问题'}>
        {
          this.state.invitationAll.map(item => {
              return(
                ...
                extra={
                  <div>
                  <a><Link to={'/updateProblem/'+item.problemId}><span style={{fontSize: '20px'}}>修改</span></Link></a>
                  &nbsp;&nbsp;
                  <a><span style={{fontSize: '20px', color: '#FF6E27'}} onClick={() => {this.delete(item.problemId)}}>删除</span></a>
                  </div>
                }>
                  <Link to={'/mobile/problemDetail/'+item.problemId}>{item.problemTitle}</Link>
                </Item>
              )
          })
        }
...
export default ManageMyProblem;